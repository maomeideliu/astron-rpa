...
public class AstronAgentServiceImpl implements AstronAgentService {
    ...
    public AppResponse<CopyRobotResponseDto> copyRobot(CopyRobotDto copyRobotDto) {
        ...
        RobotExecute sourceRobot = robotExecuteDao.selectOne(
        ...
        RobotExecute rootRobot = robotExecuteDao.selectOne(
            new LambdaQueryWrapper<RobotExecute>()
        ...
        RobotExecute existingRobot = robotExecuteDao.selectOne(
            new LambdaQueryWrapper<RobotExecute>()
    ...
    private String generateUniqueRobotName(String baseName, String userId, String tenantId, String excludeRobotId) {
        ...
        LambdaQueryWrapper<RobotExecute> queryWrapper = new LambdaQueryWrapper<RobotExecute>()
            ...
            .and(wrapper -> {
                // 匹配基础名称或基础名称+数字后缀的格式
                wrapper.eq(RobotExecute::getName, baseName)
                    .or()
                    .likeRight(RobotExecute::getName, baseName);
            });
        ...
        // 如果基础名称不存在，直接返回
        boolean baseNameExists = existingRobots.stream()
            .anyMatch(robot -> baseName.equals(robot.getName()));
        if (!baseNameExists) {
            return baseName;
        }
     ...
     * 添加组件引用列表
     */
    private void addCompUseList(String newRobotId, Integer newRobotVersion, RobotVersion authorRobotVersion, 
     ...
     * @return 参数列表
     */
    private List<ParamDto> getAllParamsWithoutAuth(String robotId, String processId, String moduleId) throws JsonProcessingException {
        // 使用不需要认证的方法获取机器人信息
     ...
     * 处理部署来源的参数
     */
    private List<ParamDto> handleDeploySource(
     ...
     * 处理部署来源的模块参数
     */
    private List<ParamDto> deployModuleHandle(
     ...
     * 处理部署来源的流程参数
     */
    private List<ParamDto> deployProcessHandle(
     ...
     * 处理市场来源的参数
     */
    private List<ParamDto> handleMarketSource(
            RobotExecute executeInfo, String processId, String moduleId) {
     ...
     * 处理市场来源的模块参数
     */
    private List<ParamDto> marketModuleHandle(
            RobotExecute executeInfo, String moduleId, String originRobotId) {
        List<CParam> params = cParamDao.getParamsByModuleId(moduleId, originRobotId, executeInfo.getAppVersion());
        return convertParams(params);
    }
     ...
     * 处理市场来源的流程参数
     */
    private List<ParamDto> marketProcessHandle(
            RobotExecute executeInfo, String processId, String originRobotId) {
        if (StringUtils.isBlank(processId)) {
            processId = cParamDao.getMianProcessId(originRobotId, executeInfo.getAppVersion());
        }
     ...
     * 处理创建来源的模块参数
     */
    private List<ParamDto> createModuleHandle(
     ...
     * 处理创建来源的流程参数
     */
    private List<ParamDto> createProcessHandle(
