...
public interface ComponentRobotUseDao extends BaseMapper<ComponentRobotUse> {
     ...
     * 根据机器人ID查询组件引用列表
     */
    List<ComponentRobotUse> getByRobotId(@Param("robotId") String robotId,
                                         @Param("tenantId") String tenantId);

    /**
     * 根据组件ID查询机器人引用列表
     */
    List<ComponentRobotUse> getByComponentId(@Param("componentId") String componentId,
                                             @Param("tenantId") String tenantId);
    ...
    @Select("select * from component_robot_use " +
            "where deleted = 0 and robot_id = #{robotId} " +
            "and robot_version = #{robotVersion} and tenant_id = #{tenantId}")
    List<ComponentRobotUse> getByRobotIdAndVersion(@Param("robotId") String robotId,
    ...
    ComponentRobotUse getByRobotIdVersionAndComponentId(@Param("robotId") String robotId,
                                                        @Param("robotVersion") Integer robotVersion,
    ...
    List<ComponentRobotUse> getByComponentIdAndVersion(@Param("componentId") String componentId,
    ...
    @Select("select * from component_robot_use where deleted = 0 and robot_id = #{robotId} " +
            ...
            "and component_version = #{componentVersion} and tenant_id = #{tenantId}")
    ComponentRobotUse getByRobotIdVersionAndComponentIdVersion(@Param("robotId") String robotId,
                                                              @Param("robotVersion") Integer robotVersion,
                                                              @Param("componentId") String componentId,
                                                              @Param("componentVersion") Integer componentVersion,
                                                              @Param("tenantId") String tenantId);
     ...
     * 删除之前的编辑态记录
     */
    @Update("update component_robot_use " +
            "set deleted = 1 " +
    ...
    List<ComponentRobotUse> getComponentRobotUse(@Param("robotId") String robotId, @Param("version") Integer version, @Param("userId") String userId);
} 