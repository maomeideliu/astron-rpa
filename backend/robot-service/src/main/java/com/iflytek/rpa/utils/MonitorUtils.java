...
//        return found ? finalDate : Calendar.getInstance().getTime();
    ...
    public static SearchTerm getSearch(){
        ...
        return new AndTerm(new ReceivedDateTerm(ComparisonTerm.GE, start),
    ...
    public static SearchTerm getSearch1(){
        ...
        return new AndTerm(new SentDateTerm(ComparisonTerm.GE, start),
                new SentDateTerm(ComparisonTerm.LE, end));
    }

    public static List<MailInfo> getMailInfos(String type, Integer port, String host, boolean sslEnable,
...
//            Store store1 = session1.getStore("pop3");
            ...
            try {
                ...
                if((mail.contains("163.com") || mail.contains("126.com")) &&
                    "imap".equals(type)
                ){
...
//                            System.out.println("bodyType------" + mp.getBodyPart(k).getContentType());
...
//                                    System.out.println("bodyType------" + mp1.getBodyPart(q).getContentType());
                                    if(mp1.getBodyPart(q).isMimeType("text/*")){
                                        String content=(String) mp1.getBodyPart(q).getContent();
                                        contentStr.append(Jsoup.parse(content).text());
//                                        System.out.println(Jsoup.parse(content).text());
                                    }
                                }
...
//                    System.out.println(DateUtils.getDayTimeFormat(messages[i].getReceivedDate()));
            ...
            finally { try{ inbox.close(false); }catch(Exception e){} try{ store1.close();if(store != null){store.close();} }catch(Exception e){} }
    ...
    public static String connectMail(String type, Integer port, String host, boolean sslEnable,
                                              String mail, String password) {
...
//            Store store1 = session1.getStore("pop3");
            ...
            finally {  try{ store1.close(); }catch(Exception e){} }
