...
public class CParamServiceImpl implements CParamService {
    ...
    @Override
    public AppResponse<List<ParamDto>> getAllParams(QueryParamDto queryParamDto) throws JsonProcessingException, NoLoginException {
...
//                    //机器人从市场中获取
//                    if (StringUtils.isBlank(robotExecute.getMarketId()) || StringUtils.isBlank(robotExecute.getAppId()) || null == robotExecute.getAppVersion()){
...
//    }
    ...
    public AppResponse<Boolean> updateParam(CParamDto cParamDto) throws NoLoginException {
        ...
        if (!StringUtils.equals(oldParamInfo.getVarName(), cParam.getVarName()) || !StringUtils.equals(oldParamInfo.getVarDescribe(), cParam.getVarDescribe()) || !StringUtils.equals(oldParamInfo.getVarType(), cParam.getVarType()) || !StringUtils.equals(oldParamInfo.getVarValue(), cParam.getVarValue()) || oldParamInfo.getVarDirection() != cParam.getVarDirection()) {
            cParam.setUpdaterId(userId);
            cParam.setUpdateTime(new Date());
    ...
    public AppResponse<Boolean> saveUserParam(CParamListDto cParamListDto) throws NoLoginException, JsonProcessingException {
    ...
    private AppResponse<Boolean> updateExecutorParamDetail(RobotExecute robotExecute, List<CParam> cParamList) throws NoLoginException, JsonProcessingException {
        AppResponse<User> response = rpaAuthFeign.getLoginUser();
        if (response == null || !response.ok()) {
            throw new ServiceException("用户信息获取失败");
        }
