...
public interface SharedFileDao extends BaseMapper<SharedFile> {
    ...
    @Select("SELECT COUNT(*) FROM shared_file_tag WHERE tag_name = #{tagName} AND tenant_id = #{tenantId} AND deleted = 0")
    ...
    @Select("SELECT EXISTS(SELECT 1 FROM shared_file_tag WHERE tag_id = #{tagId} AND tenant_id = #{tenantId} AND deleted = 0)")
