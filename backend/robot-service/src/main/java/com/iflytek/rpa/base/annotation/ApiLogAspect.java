...
public class ApiLogAspect {
     ...
     * 记录方法调用开始信息
     */
    private void logMethodStart(String requestId, String className, String methodName, 
                               Object[] args, ApiLog apiLog) {
        try {
            ...
            log.warn("[{}] [API日志异常] requestId={} message={}", 
                LocalDateTime.now().format(TIMESTAMP_FORMATTER), requestId, e.getMessage());
        }
    }
     ...
     * 记录方法调用成功信息
     */
    private void logMethodSuccess(String requestId, String className, String methodName, 
                                 Object result, long executionTime, ApiLog apiLog) {
        try {
            String timestamp = LocalDateTime.now().format(TIMESTAMP_FORMATTER);
            StringBuilder logMsg = new StringBuilder();
     ...
     * 记录方法调用异常信息
     */
    private void logMethodException(String requestId, String className, String methodName, 
                                   Throwable throwable, long executionTime, ApiLog apiLog) {
        try {
            String timestamp = LocalDateTime.now().format(TIMESTAMP_FORMATTER);
            StringBuilder logMsg = new StringBuilder();
