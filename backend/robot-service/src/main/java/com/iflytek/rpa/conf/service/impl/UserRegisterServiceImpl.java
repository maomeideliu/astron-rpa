...
//                AppResponse<UserRegisterVo> response = AppResponse.error(ErrorCodeEnum.E_COMMON, "注册失败：账号已存在");
...
//                AppResponse<UserRegisterVo> response = AppResponse.error(ErrorCodeEnum.E_COMMON, "注册失败：未找到默认密码配置");
//                return response;
//            }
//
...
//            if (userResult <= 0) {
//                AppResponse<UserRegisterVo> response = AppResponse.error(ErrorCodeEnum.E_COMMON,"注册失败：插入用户信息失败");
//                return response;
//            }
//
//            // 插入租户用户关系表
//            int tenantUserResult = uapUserDao.insertTenantUser(uapDatabaseName, tenantUserId, defaultTenantId, userId);
//            if (tenantUserResult <= 0) {
//                AppResponse<UserRegisterVo> response = AppResponse.error(ErrorCodeEnum.E_COMMON,"注册失败：插入租户用户关系失败");
//                return response;
//            }
//
//            int roleUserResult = uapUserDao.insertRoleUser(uapDatabaseName, roleUserId, defaultRoleId, defaultTenantId, userId);
//            if (roleUserResult <= 0) {
//                AppResponse<UserRegisterVo> response = AppResponse.error(ErrorCodeEnum.E_COMMON,"注册失败：插入角色用户关系失败");
//                return response;
//            }
//
//            // 返回账号和默认密码（明文）
...
//        } catch (Exception e) {
//            AppResponse<UserRegisterVo> response = AppResponse.error(ErrorCodeEnum.E_COMMON,"注册失败：" + e.getMessage());
//            return response;
//        }
    }
}
