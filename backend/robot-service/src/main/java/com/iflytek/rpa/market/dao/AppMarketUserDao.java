...
public interface AppMarketUserDao extends BaseMapper<AppMarketUser> {
    ...
    List<MarketDto> getUserByPhoneForOwner(@Param("entity") MarketDto marketDto, @Param("marketId") String marketId, @Param("userId") String userId, @Param("databaseName") String databaseName);
    ...
    @Select("select id from app_market_user where deleted=0 and market_id=#{marketId} and creator_id=#{creatorId} and user_type != 'owner' and deleted = 0")
    String getIdByMarketIdAndCreatorId(@Param("marketId") String marketId, @Param("creatorId") String creatorId);

    Integer deleteById(@Param("id") String id);

    IPage<MarketDto> getUserListByPublic(IPage<MarketDto> pageConfig, @Param("entity") MarketDto marketDto, @Param("nowUserid") String nowUserid,@Param("databaseName") String databaseName);
