    ...
    private void timeTransToCron(ScheduleTask task){
        ...
        if("cycle".equals(task.getRunMode())){
            ...
            if("month".equals(task.getScheduleType())){
                ...
                //每周固定时间点
                cron = rule.getSecond() + " " + rule.getMinute() + " " + rule.getHour() + " ? * " + CycleWeekEnum.getCodeByNum(rule.getDayOfWeek());
...
//        List<TaskRobotBo> allRobotList = new ArrayList<>();
        ...
        //机器人根据taskId分组
        Map<String, List<ScheduleTaskRobot>> scheduleTaskRobotMap = scheduleTaskRobotList.stream().collect(Collectors.groupingBy(ScheduleTaskRobot::getTaskId));
...
//                continue;
//            }
//            List<TaskRobotBo> taskRobotList = JSONObject.parseArray(task.getExecuteSequence(), TaskRobotBo.class);
...
//        }
//        //根据robotId分组
//        Map<String, String> robotVersionMap = robotVersionList.stream().collect(Collectors.toMap(RobotVersion::getRobotId,RobotVersion::getName));
        //设置名字
        for (ScheduleTask task: taskList) {
    ...
    public AppResponse<?> saveTask(ScheduleTaskDto task) throws NoLoginException {
        ...
        if(StringUtils.isNotBlank(task.getTaskId())){
            ...
            if (count > 0){
                ...
                //查询历史机器人
                List<String> hisTaskRobotIdList = scheduleTaskRobotDao.queryHisRobotIdListByTaskId(scheduleTask.getTaskId());
                //是更新操作，且数据修改了，则先删后增
