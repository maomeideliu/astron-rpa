...
public interface ComponentRobotBlockDao extends BaseMapper<ComponentRobotBlock> {
     ...
     * 根据机器人ID和组件ID查询屏蔽记录
     */
    ComponentRobotBlock getBlockByRobotAndComponent(@Param("robotId") String robotId,
     ...
     * 根据机器人ID查询所有屏蔽的组件
     */
    List<ComponentRobotBlock> getBlocksByRobotId(@Param("robotId") String robotId,
                                                 @Param("tenantId") String tenantId);

    /**
     * 根据组件ID查询所有屏蔽该组件的机器人
     */
    List<ComponentRobotBlock> getBlocksByComponentId(@Param("componentId") String componentId,
                                                     @Param("tenantId") String tenantId);
    ...
    Long checkBlockExists(@Param("robotId") String robotId,
     ...
     * 逻辑删除屏蔽记录
     */
    @Update("update component_robot_block " +
            "set deleted = 1, updater_id = #{updaterId}, update_time = now() " +
            "where id = #{id} and tenant_id = #{tenantId}")
    Integer deleteBlock(@Param("id") Long id,
    ...
    Integer deleteBlockByRobotAndComponent(@Param("robotId") String robotId,
                                           @Param("robotVersion") Integer robotVersion,
                                           @Param("componentId") String componentId,
                                           @Param("updaterId") String updaterId,
                                           @Param("tenantId") String tenantId);
    ...
    @Select("select component_id from component_robot_block " +
            ...
            "and deleted = 0 and tenant_id = #{tenantId}")
    List<String> getBlockedComponentIds(@Param("robotId") String robotId,
                                        @Param("robotVersion") Integer robotVersion,
                                        @Param("tenantId") String tenantId);
    ...
    List<ComponentRobotBlock> getComponentRobotBlock(@Param("robotId") String robotId, @Param("version") Integer version, @Param("userId") String userId);
    ...
    List<ComponentRobotBlock> getComponentRobotBlockForCopy(@Param("robotId") String robotId, @Param("version") Integer version, @Param("tenantId") String tenantId);
