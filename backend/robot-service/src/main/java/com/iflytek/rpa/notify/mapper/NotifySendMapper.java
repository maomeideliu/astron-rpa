...
public interface NotifySendMapper extends BaseMapper<NotifySend> {
    ...
    @Select("select message_info, message_type, create_time, operate_result  " +
            "from notify_send " +
    ...
    @Select("select id, message_info, message_type, create_time, operate_result  " +
            "from notify_send " +
            "where deleted=0 and tenant_id=#{tenantId} and user_id=#{userId} and (operate_result=1 or operate_result=2) " +
            "order by create_time desc " +
            "limit #{pageNo}, #{pageSize}")
    List<NotifyVo> getNotifyListVo(@Param("tenantId") String tenantId,
    ...
    // 一键已读，update语句
    @Update("update notify_send set operate_result=2 where user_id=#{userId} and tenant_id=#{tenantId} and operate_result=1")
    ...
    @Select("select count(id) from notify_send where deleted=0 and user_id=#{userId} and tenant_id=#{tenantId} and operate_result=1")
