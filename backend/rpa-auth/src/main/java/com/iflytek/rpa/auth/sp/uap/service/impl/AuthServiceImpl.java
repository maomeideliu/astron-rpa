...
public class AuthServiceImpl implements AuthService {
     ...
     */
    @Override
    public AppResponse<List<com.iflytek.rpa.auth.core.entity.TreeNode>> getUserAuthTreeInApp(HttpServletRequest request){
        //当前登录用户在应用中的菜单信息
     ...
     */
    @Override
    public AppResponse<com.iflytek.rpa.auth.core.entity.TreeNode> getAuthResourceTreeInApp(String roleId, HttpServletRequest request){
        if(null == roleId){
        ...
        //勾选的菜单数据根据菜单id分组
        Set<String> checkedAuthIdSet = checkedAuthList.stream().map(UapAuthority::getId).collect(Collectors.toSet());
        //根据id和parentId组装菜单树
        ...
        //查询勾选的资源数据
        ResponseDto<List<UapResource>> checkedResourceListResponse =  UapManagementClientUtil.queryResourceListByRoleId(roleId, request);
        ...
        //根据resourceId分组
        Set<String> checkedResourceIdSet = checkedResourceList.stream().map(UapResource::getId).collect(Collectors.toSet());
        // 多线程异步查询资源权限
        ...
        for (String leafNodeAuthId : leafNodeAuthIdList) {
            CompletableFuture<Void> future = CompletableFuture.supplyAsync(() -> {
                    ...
                    .thenAcceptAsync(checkResourceList -> {
                        // 转换为TreeNode并设置勾选状态
                        List<TreeNode>  resourceNodeList = new ArrayList<>();
                        for(UapResource resource:checkResourceList){
                            TreeNode treeResourceNode = convertResourceToTreeNode(resource, leafNodeAuthId, NODE_TYPE_RESOURCE);
                            treeResourceNode.setChecked(checkedResourceIdSet.contains(treeResourceNode.getId()));
    ...
    public TreeNode buildTree(String rootId, List<UapAuthority> menuList,
                              Set<String> checkedAuthIdSet, List<String> leafNodeAuthIdList) {
    ...
    private static final ExecutorService RESOURCE_QUERY_EXECUTOR = Executors.newFixedThreadPool(
            ...
            new ThreadFactoryBuilder().setNameFormat("RoleManage-resource-query-thread-%d").build()
    );
    ...
    public AppResponse<String> saveRoleAuth(RoleAuthResourceDto roleAuthResourceDto, HttpServletRequest request){
        //获取保存前的菜单权限树
        AppResponse<com.iflytek.rpa.auth.core.entity.TreeNode> originalTreeResponse = getAuthResourceTreeInApp(roleAuthResourceDto.getRoleId(), request);
...
//        log.info("返回响应：{}, {}",canceledResources,canceledMenus);
        ...
        if(!CollectionUtil.isEmpty(canceledResources)){
            ...
            ResponseDto<Object> unBindRoleResourceResponse = UapManagementClientUtil.unBindRoleResource(tenantId, bindResourceDto, request);
        ...
        if(!CollectionUtil.isEmpty(authorityResourceList)){
            ...
            ResponseDto<Object> saveResponse = ClientManagementAPI.bindRoleAuthResource(UapUserInfoAPI.getTenantId(request), bindRoleAuthResourceDto);
