...
class XcApiKeyService:
    def __init__(self, db: AsyncSession, redis: Redis = None):
        self.db = db
        self.redis = redis

    async def create_xcagent(self, xcagent_data, user_id: str):
        """创建星辰Agent"""
        from app.models.api_key import XCAgentDB

        # 创建新的星辰Agent记录
        new_xcagent = XCAgentDB(
            user_id=user_id,
            xc_user_name=xcagent_data.user_name,
            app_id=xcagent_data.app_id,
            api_key=xcagent_data.api_key,
            api_secret=xcagent_data.api_secret,
            created_at=datetime.now(pytz.timezone("Asia/Shanghai")).replace(tzinfo=None),
            updated_at=datetime.now(pytz.timezone("Asia/Shanghai")).replace(tzinfo=None),
            is_active=1,
        )

        self.db.add(new_xcagent)
        await self.db.flush()
        await self.db.refresh(new_xcagent)

        logger.info(f"Created XCAgent for user {user_id}: {new_xcagent.id}")
