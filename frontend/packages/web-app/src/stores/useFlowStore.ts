...
import { Catch, CvImageExist, CvImageExistEnd, FileExist, FolderExist, ForBrowserSimilar, ForDict, ForEnd, ForExcelContent, ForList, ForStep, Group, GroupEnd, If, IfEnd, Try, TryEnd, While, WindowExist } from '@/views/Arrange/config/atomKeyMap'
...
export const useFlowStore = defineStore('flow', () => {
  ...
  const generateContactMap = (list: RPA.Atom[]) => {
    ...
    list.forEach((i) => {
      ...
      if (i.key === Group)
        groupList.push(i)
      else if ([ForStep, ForDict, ForList, ForExcelContent, ForBrowserSimilar, While].includes(i.key))
        forList.push(i)
