...
from astronverse.executor import ExecPosition, ExecuteStatus, ProjectInfo, ProcessInfo
...
from astronverse.executor.flow.syntax.environment import EnvItem, EnvBizTypes
from astronverse.executor.flow.syntax.event import event, CloseError, EventKey
...
class Executor:
    ...
    def debug_handler(self, svc, env, token):
        ...
        self.svc.report.info(
            ReportCode(
                ...
                debug_data={"is_break": is_break, "data": env.to_json_dict(project_id) if is_break else None},
            )
        )
    ...
    def append_child_flow_list(self, flow, flow_list, read_position):
        ...
        if flow.get("key") == TokenType.Process.value and "__temp_process_end__" not in flow:
            ...
            child_flow_list = self.get_flow_list(
                project_id, new_process_id, project_info.project_mode, project_info.project_version
            )
            ...
            pre = flow_list[: read_position + 1]
            child_flow_list = self.get_flow_list(
                new_project_id, new_process_id, new_project_info.project_mode, new_project_info.project_version
            )
            pre.extend(child_flow_list)
    ...
    def project_init(
        ...
        def raw_project_init():
            ...
            # 日志1
            self.svc.report.info(
                ReportFlow(log_type=ReportType.Flow, status=ReportFlowStatus.INIT, msg_str=ReportFlowInit)
            )
            ...
            for p in self.svc.project_dict.values():
                ...
                for i, v in enumerate(process_list):
                    process_dict[v.get("processId")] = ProcessInfo(
                        name=v.get("processName"), project_id=p.project_id, process_id=v.get("processId")
                    )
                    if i == 0:
                ...
                if p.project_id == project_id:
                    if process_id:
                        if process_id not in process_dict:
                            raise BaseException(ENGINEERING_DATA_ERROR, "工程数据异常 {}".format(process_id))
                    else:
            ...
            for p in self.svc.project_dict.values():
                ...
                env.setitem(
                    p.project_id,
                    "__importlib__",
                    EnvItem(biz_types=EnvBizTypes.Env, types="Any", key="__importlib__", value=importlib),
                )
                    ...
                    for g_v in global_list:
                        ...
                        env.setitem(
                            ...
                            EnvItem(
                                ...
                                ext={"globalId": g_v.get("globalId"), "varDescribe": g_v.get("varDescribe")},
                            ),
                        )
            ...
            self.svc.report.info(
                ReportFlow(
                    log_type=ReportType.Flow, status=ReportFlowStatus.INIT_SUCCESS, msg_str=ReportFlowInitSuccess
                )
            )
            return program
    ...
    def __process_run__(self, program, env=None):
        ...
        self.svc.report.info(
            ReportFlow(log_type=ReportType.Flow, status=ReportFlowStatus.TASK_START, msg_str=ReportFlowTaskStart)
        )
        try:
