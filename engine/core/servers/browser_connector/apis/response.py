...
from browser_connector.error import CODE_INNER, CODE_OK, BaseException
from browser_connector.logger import logger
...
class CustomResponse:
    ...
    def tojson(cls, data: Any = None):
        return JSONResponse(
            cls(CODE_OK.code.value, CODE_OK.message, data=data).__dict__,
            status_code=CODE_OK.httpcode,
        )
...
async def http_exception(request: Request, exc: Exception):
    ...
    return JSONResponse(
        CustomResponse(CODE_INNER.code.value, CODE_INNER.message, {}).__dict__,
        status_code=CODE_INNER.httpcode,
    )
...
async def http_base_exception(request: Request, exc: BaseException):
    ...
    return JSONResponse(
        CustomResponse(exc.code.code.value, exc.code.message, {}).__dict__,
        status_code=exc.code.httpcode,
    )
