...
class DocxCore(IDocxCore):
    word_obj = None

    @staticmethod
    def _word_is_exists():
        word_flag, wps_flag = False, False
    ...
    def _create_app(params: str):
        try:
            word_obj = win32com.client.gencache.EnsureDispatch(params)
            return word_obj
        except Exception as err:
            try:
                word_obj = win32com.client.Dispatch(params)
                return word_obj
            except Exception as err:
    ...
    def init_word_app(cls, default_application: ApplicationType = ApplicationType.DEFAULT):
        ...
        for key in keys:
            try:
                cls.word_obj = cls._create_app(key)
                if cls.word_obj:
                    return cls.word_obj
    ...
    def open(
        ...
        :param encoding: 编码格式
        :param open_pwd: 打开密码
        :param write_pwd: 写入密码
        :return: word对象
        """
        if cls.word_obj is None:
            cls.init_word_app(default_application)
            # cls.word_obj.Visible = True
        if visible_flag:
            cls.word_obj.Visible = True
        if file_path:
            cls.ScreenUpdating = True
            cls.word_obj.DisplayAlerts = False
            doc = cls.word_obj.Documents.Open(
                FileName=file_path,
            ...
            cls.word_obj.DisplayAlerts = True
            print(doc.Name)
        else:
    ...
    def read(cls, doc: object, select_range=SelectRangeType.ALL):
        ...
        """
        if not cls.word_obj:
            cls.word_obj = doc.Application
            cls.word_obj.Visible = True
        if select_range == SelectRangeType.SELECTED:
            s = cls.word_obj.Selection
            return re.sub(r"\r", "\n", s.Text)
        else:
            data = ""
            for p in doc.Paragraphs:
                temp_text = re.sub(r"\r", "", p.Range.Text) + "\n"
                data += temp_text
            return data
    ...
    def create(
        ...
        doc = cls.word_obj.Documents.Add(Visible=visible_flag)
        if new_file_path:
            doc.SaveAs(FileName=new_file_path)
    ...
    def cursor_position(
        ...
        if by == SelectTextType.CONTENT:  # 按照文本定位
            ...
            try:
                ...
                raise BaseException(CONTENT_FORMAT_ERROR_FORMAT, "文档为空！")
            ...
            try:
                ...
                raise BaseException(CONTENT_FORMAT_ERROR_FORMAT, "内容为空！")
    ...
    def read_table(
        ...
        if search_type == SearchTableType.IDX:
            try:
                ...
                raise BaseException(WORD_TABLE_EXIST_ERROR.format("序号" + str(idx)))
            ...
            if not table_content:
                raise BaseException(WORD_TABLE_EXIST_ERROR.format("内容" + str(text)))
    ...
    def insert_table(
        ...
        # 插入表格
        table = doc.Tables.Add(range, NumRows=rows, NumColumns=cols)
    ...
    def convert_to_txt(
        ...
        if DocxCore.check_file_in_path(output_path, filename):
            if save_type == SaveFileType.WARN:
                raise BaseException(FILENAME_ALREADY_EXIST_ERROR.format(filename), "")
            elif save_type == SaveFileType.GENERATE:
                # 生成非重复文件名
                counter = 1
                oldfilename, _ = os.path.splitext(filename)
                while DocxCore.check_file_in_path(output_path, filename):
